  ...
  chooseImage(source) {
    Taro.chooseImage({
      ...
      success: function(res) {
        let path = '/pages/result/result?img=' + res.tempFilePaths[0]
        Taro.navigateTo({
          url: path
        })
      }
      ...
      <View className='home'>
        <View className='at-row at-row__justify--around'>
          <View className='at-col at-col-5'>
            <AtButton type='primary' onClick={this.chooseImage.bind('camera')}> 拍照记录</AtButton>
          </View>
          <View className='at-col at-col-5'>
            <AtButton type='secondary' onClick={this.chooseImage.bind('album')}>相册导入</AtButton>
          </View>
        </View>
